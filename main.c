...
int main (int argc, char *argv[])
{
    ...
    {
        D=recebeDicionarioOrdenado(argv[1],&comp);                                       //passa argumento 2(nome do dicionario) para fun��o que o coloca em uma LDE
        ...
        while(!feof(entrada)){                                                          //enquanto n�o chega no final do arquivo de entrada(texto a ser lematizado)
            ...
            posicao=encontraNoDicOrdenado(D,palavra,&comp);                                     //chama fun��o que procura posi��o da palavra na lista(se existir) e a devolve
